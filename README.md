...
- ランク制度、実績(191種)、統計、ランキング(Supabase連携)、多数の隠しイースターエッグ・隠し部屋(19部屋+ストーリー)
- 管理者ブロードキャスト機能(バナー配信、パスワードはSHA-256ハッシュで保管)

### 隠しストーリーについて

地下室(`basement`)を起点に、起源記録・創設者の記録・機密ファイル・監視ログなど17の隠し部屋を巡ると最終部屋「真実」(`final`)に到達する。さらに機密ファイル室/放送局の秘密チャンネルから「謎の人物ファイル(`mystery_person`)」→「暗号解読室(`cipher`)」を発見・解読すると、「真実」画面に隠しボタンが出現し「エピローグ(`epilogue`)」へ進める、という2段構えの隠しストーリーになっている。

加えて、消去ファイル室(`archive_c`)には唯一「復元できなかった」とされていたFILE_∞.txtを復元する「FILE_∞の真実(`file_infinity`)」があり、このゲームの本当の目的が明かされる。また"neko"と入力すると「トリッピの正体(`trippi`)」という、隠語の世界になぜか猫がいる理由を明かす小部屋に飛べる（数少ない「ちゃんと解決する」謎）。この2つは「全部屋制覇」の必須部屋には含めていない任意のボーナス部屋という位置づけ。

新しい部屋・実績を追加する際は既存の世界観(SIBAKO語・創始者スビニィナボンバルディロ・「記録するな。記憶せよ。」の法則など)と矛盾しないようにすること。ストーリー全体の詳細は Claude 側の記憶(`memory/ingo-kuizu-story.md`)にも保存してある。
...
js/
  ...
  rooms/            隠し部屋(イースターエッグ)ごとのHTML生成
    ...
    epilogue.js        エピローグ
    file-infinity.js   FILE_∞の真実（このゲームの本当の目的）
    trippi.js          トリッピの正体
    misc-rooms.js
